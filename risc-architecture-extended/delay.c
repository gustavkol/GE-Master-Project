...
void compare_and_iter(int n_prev, int *n_next, signed int a_prev, signed int *a_next, 
    ...
    //iprintf("n_prev + a_prev = %d, inc_term_w_error - inc_term_prev = %d\n", in1, in2);

    _OA_RV_COMPARE_AND_ITER(n_prev, in2, out);

    //!!!! look at other instruction formats for several inputs/outputs !!!!!//
    ...
    *n_next         = n_prev + *a_next;
    ...
    //iprintf("delta_a = %hhd, compensated = %d, error = %d\n\n", delta_a, compensated, *error_next);
    //iprintf("a_next = %d, inc_term_next = %d, n_next = %d, error_next = %d\n\n", *a_next, *inc_term_next, *n_next, *error_next);
}
