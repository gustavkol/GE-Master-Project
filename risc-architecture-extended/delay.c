...
void init_delay(unsigned char r_0, unsigned char angle, int *delay_array, int *inc_term_array) {
    ...
    int x_scale;                        
    _OA_RV_CONST_MULT_C0(r_0, 0, x_scale);                  //  2*p*(f_s/v_s)^2*r_0
...
void compare_and_iter(int n_prev, int *n_next, signed int *a_next, signed int *inc_term_next, signed int *error_next, signed int inc_term) {
    ...
    _OA_RV_COMPARE_AND_ITER(n_prev, inc_term, out);
                                                            //signed int compensated    = out >> 8;
                                                            //signed char delta_a       = out & 0xFF;
    _OA_RV_SHIFT_ADD(-out, inc_term,*error_next);           //*error_next               = in2 - compensated;
    _OA_RV_SHIFT_ADD(out, 0,*inc_term_next);                //*inc_term_next            = compensated;
    _OA_RV_MASK_ADD(out, 0, *a_next);                       //*a_next                   = delta_a;
...
void compare_and_iter_next(int *n_prev, signed int *a_prev, signed int *inc_term_prev, signed int *error_prev, signed int inc_term) {
    ...
    _OA_RV_SHIFT_ADD(out, 0,*inc_term_prev);                //*inc_term_next            = compensated + inc_term_prev;
    _OA_RV_MASK_ADD(out, 0, *a_prev);                       //*a_next                   = a_prev + delta_a;
...
void increment_and_compare_next(int *n_prev, signed int *a_prev, signed int *inc_term_prev, signed int *error_prev, signed int inc_term) {
    ...
    if (sign_bit == -1) {
        while (comp_term > inc_term_w_error) {
            a -= INC_STEP;                                  
            //_OA_RV_COMP_TERM_ITER(a, n_prev, iter_term); 
            iter_term = 2*(*n_prev + a)*INC_STEP + 0b0010;
            comp_term -= iter_term;                        
        }
        comp_term += iter_term;
        a += INC_STEP;
